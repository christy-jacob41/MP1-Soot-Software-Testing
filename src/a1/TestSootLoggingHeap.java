		...
		if(!b.getMethod().getDeclaringClass().getName().equals("a1.Log"))
		{
		    ...
		    while(it.hasNext()){
		    	...
		    	if (stmt.containsFieldRef()) {
		    		...
		    		if(stmt.getDefBoxes().toString().contains("LinkedVariableBox($"))
		    			isWrite = false;
		    					 
		    		boolean isStatic = stmt.getFieldRef().getField().isStatic();
		    				    		
		    		a1.Log.logFieldAcc(Thread.currentThread(), stmt.getFieldRef().getField().getSignature(), isStatic, isWrite);
